...
镜像 box_breakout（放量箱体突破买入信号），共享箱体识别函数：
  - 箱体完整形成（上沿触碰≥min_touches 且下沿触碰≥min_touches 且时长≥box_min_days）
  - 收盘价跌破下沿（不设容差，敏感优先）
  - 事件生命周期：单日跌破 warning → 连续 2 日 strong_sell → 站回下沿清除

与买入侧的关键差异：
  - 不检测成交量（阴跌比放量下跌更常见、更阴险）
  - 同一箱体同一时间只存在一个跌破事件（事件演化而非重复创建）

信号等级：
  strong_sell — 连续 2 日收盘 < 下沿
  warning     — 单日收盘 < 下沿（事件创建）
...
def detect(daily: List[Dict], params: Optional[Dict] = None) -> List[Dict]:
    ...
    for t in range(params['lookback_days'] + params['local_window'] + 1, n):
        ...
        # 未匹配：对所有活跃事件做站回检查（收盘 ≥ 事件下沿 → 清除），不依赖当天带识别
        for k in list(active.keys()):
            ev_k = active[k]
            if closes[t] >= ev_k['band_bottom']:
                ev = active.pop(k)
                ev['result'] = 'failed'
                ev['end_reason'] = '站回下沿'
                events.append(ev)
        ...
        if matched_key is not None and matched_key not in active:
            matched_key = None  # 事件已被外层站回清除
        if matched_key is None:
            continue  # 无活跃事件需处理（新箱体冻结在未匹配分支已处理）
        key = matched_key
        ev = active[key]
